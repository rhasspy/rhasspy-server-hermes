...
pyinstaller:
	mkdir -p dist
	pyinstaller -y --workpath pyinstaller/build --distpath pyinstaller/dist rhasspyserver_hermes.spec
	tar -C pyinstaller/dist -czf dist/rhasspy-server-hermes_$(version)_$(architecture).tar.gz rhasspyserver_hermes/
...
debian: pyinstaller
	...
	cp -R pyinstaller/dist/rhasspyserver_hermes "$(debian_dir)/usr/lib/"
